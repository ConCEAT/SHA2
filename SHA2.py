...
class Hash:
    ...
    def __messageCreator(self, object):
        string = str(object)
    ...
    def __wordsCreator(self, message):
        words = []
        for x in xrange(16):
    ...
    def __bowelsLoop(self, words):
        a, b, c, d, e, f, g, h = self.keys
        for i in xrange(64):
            s0 = (rightRotate(e, 6) ^ rightRotate(e, 11)) ^ rightRotate(e, 25)
            ch = (e & f) ^ (Not(e) & g)
            t0 = Add(Add(Add(Add(h, s0), ch), self.constants[i]), words[i])
    ...
    def __digest(self):
        result = "".join([hex(key)[2:].replace("L", "").zfill(8) for key in self.keys])
        return "0x" + result

    #---------------------MAIN FUNCTION---------------------#

    def __init__(self, object):
        binMessage = self.__messageCreator(object)
        blocks = len(binMessage)/512
        for index in xrange(blocks):
            binMessage_block = binMessage[512*index: 512*index+512]
            words = self.__wordsCreator(binMessage_block)
            blockExitValues = self.__bowelsLoop(words)
            for i in xrange(8):
                self.keys[i] = Add(self.keys[i], blockExitValues[i])
